...
for n, beta in enumerate(beta_values, start=1):
    ...
    pattern = f"../../raw_data/Nf{flavours}_hot/hmc_nf{flavours}_*b{int(beta*10)}*.out"
    ...
    for filename in files:
        ...
        os.system(f"grep 'Plaquette' {filename} | awk '{{print $NF}}' > ../../data/Nf{flavours}_hot/plaquette_lines.txt")

        # Read in the temporary file
        with open(f"../../data/Nf{flavours}_hot/plaquette_lines.txt", "r") as f:
            lines = f.readlines()
        ...
        os.remove(f"../../data/Nf{flavours}_hot/plaquette_lines.txt")

    # Write the results to the output file in ascending order based on the first column
    with open(f"../../data/Nf{flavours}_hot/bulktrans_sp4_2AS_hot_{n}.dat", "w") as f:
